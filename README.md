...
`install.sh` 実行後、WSL内で以下を叩いて全項目を一括確認できる。

```bash
echo "=== .zshrc ===" && ls -la ~/.zshrc
echo "=== .zshrc.local ===" && ls ~/.zshrc.local && echo "exists"
echo "=== wsl.conf ===" && cat /etc/wsl.conf
echo "=== git include ===" && git config --global --get-all include.path
echo "=== git alias st ===" && git config --global alias.st
echo "=== wezterm ===" && ls /mnt/c/Users/$(cmd.exe /c "echo %USERNAME%" 2>/dev/null | tr -d '\r\n')/.config/wezterm/
```

**期待する出力:**
| 確認項目 | 期待値 |
|---|---|
| `.zshrc` | `~/.zshrc -> .../dotfiles/.zshrc`（シンボリックリンク） |
| `.zshrc.local` | `exists` と表示される |
| `wsl.conf` | `[interop]` セクションが出力される |
| `git include` | `~/dotfiles/git/gitconfig.shared` のパスが表示される |
| `git alias.st` | `status` と表示される |
| `wezterm` | `keybinds.lua` と `wezterm.lua` が表示される |

WezTermは起動して見た目が壊れていなければOK。
